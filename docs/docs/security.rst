...
Version 3 of RPyC-NG was a rewrite of the library, specifically targeting security and
service-orientation. Unlike version 2.6, RPyC-NG no longer makes use of insecure protocols like ``pickle``,
...
take a layered to secure programming and not let RPyC-NG be a single point of failure.
...
RPyC-NG is intuitive and secure when used properly. However, if not used properly, RPyC-NG is also the perfect back-door...
The general recommendation is not to use RPyC-NG openly exposed over the Internet. It's wiser to use it only over secure local
...
Of course, you can use RPyC-NG over a :ref:`secure connection <ssl>`, to mitigate these risks.

RPyC-NG works by exposing a root object, which in turn may expose other objects (and so on). For
...
``sys`` modules. The default configurations for RPyC-NG are intended to mitigate access to dangerous objects. But if you enable
...
Since this is a common idiom, RPyC-NG provides :func:`~rpyc.utils.helpers.restricted`.
...
Assuming RPyC-NG is configured to allow access only to safe attributes (the default), this would
be secure.
...
------------------------
By default, RPyC-NG is configured to allow very little attribute access. This is useful when your
...
Attribute Access
----------------
RPyC-NG has a rather elaborate attribute access scheme, which is controlled by configuration
parameters. However, in case you need more fine-grained control, or wish to completely override
the configuration for some type of objects -- you can implement the **RPyC-NG attribute protocol**.
