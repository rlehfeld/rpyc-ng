...
<https://en.wikipedia.org/wiki/Address_space>`_ -- a region of (virtual) memory,
...
`remote procedure calls <https://en.wikipedia.org/wiki/Remote_procedure_call>`_.
...
describe what functions are exposed, define a `serialization <https://en.wikipedia.org/wiki/Serialization>`_
...
`callback functions <https://en.wikipedia.org/wiki/Callback_(computer_science)>`_.
...
* `By Value <https://en.wikipedia.org/wiki/Evaluation_strategy#Call_by_value>`_ -
...
Object Proxying
^^^^^^^^^^^^^^^
`Object proxying <https://en.wikipedia.org/wiki/Proxy_pattern>`_ is a technique of referencing
