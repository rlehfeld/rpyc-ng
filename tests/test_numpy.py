...
class TestNumpy(unittest.TestCase):
    def setUp(self):
        self.server = rpyc.utils.server.OneShotServer(MyService, port=0, protocol_config={"sync_request_timeout": 60, "allow_pickle": True})
        self.server.logger.quiet = False
        self.thd = self.server._start_in_thread()
        self.conn = rpyc.connect("localhost", port=self.server.port, config={"sync_request_timeout": 60, "allow_pickle": True})
